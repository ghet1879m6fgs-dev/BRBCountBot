...
class SalesBot:
    ...
    def migrate_old_data(self):
        ...
        # Маппинг неправильных ключей в правильные
        bad_to_good = {}
            ...
            for user_str, user_data in data.items():
                ...
                for k, v in sales.items():
                    if k in expected_keys:
                        ...
                        continue
                    # Прямое преобразование неправильных ключей
                    if k in bad_to_good:
                        good = bad_to_good[k]
                        new_sales[good] = new_sales.get(good, 0) + v
                        changed = True
                        continue
                    # Эвристика: разделение по '_' и поиск соответствия
                    if '_' in k:
                        ...
                        if prefix in TARIFFS:
                            ...
                            if found:
                                ...
                                changed = True
                                continue
                    # Запасной вариант: оставить оригинальный ключ
                    new_sales[k] = new_sales.get(k, 0) + v
        ...
        # Применяем миграцию к существующим JSON файлам
    ...
    async def record_sale(self, query, record_key, display_name: str = None):
        """Запись продажи с сохранением в файлы"""
        ...
        # Определение отображаемого имени
        if not display_name:
            ...
            if prefix in TARIFFS:
                ...
                for sub_key, human_name in submenu.items():
                    if human_name == suffix or human_name.strip() == suffix.strip():
                        return sub_key
        # Поиск по человекочитаемым именам
        for t, info in TARIFFS.items():
            submenu = info.get('submenu') or {}
            for sub_key, human_name in submenu.items():
    ...
    def get_display_name(self, tariff_key):
        """Преобразовать ключ тарифа в читаемое название"""
        display_map = {
    ...
    async def show_reset_options(self, query, manager_id=None):
        ...
        if manager_id:
            manager_data = self.sales_data.get(int(manager_id))
            if manager_data:
    ...
    async def show_calculator(self, query, user_id):
        """Показать расчет суммы всех проданных тарифов"""
        user_data = self.sales_data[user_id]
